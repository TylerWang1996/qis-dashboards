...
Place the optional correlation heatmap after the ranked tables. Keep its color scale fixed at [-1, 1]. Appendix I.6 specifies deterministic correlation clustering for display, while the model's configured universe order remains unchanged. Full pair and strategy tables are available in the appendix.
...
| 7. Review and delivery | Full checks; documented benchmark; clean committed notebook; verified private main push | Complete: 98 tests and Ruff pass; all independent review findings resolved; performance targets met; source notebook has no outputs. Private-main delivery verified and GitHub Actions passed (initial delivery run 34163651539). Final closure commit is verified in the task handoff. |
| 8. Presentation update | Clustered official matrix, signed strategy waterfall, visible glossary, capitalization, regression and visual checks | Complete: 114 tests, Ruff, locked sync, independent review, static notebook/HTML visuals, and performance targets pass. Implementation checkpoint 093d757 was verified on private main; GitHub Actions run 34165492534 passed. |
...
#### I.6 Approved presentation update (2026-09-07)

The approved presentation update extends the PM flow to Current Diversification, Strategy Risk Contributions, Monthly Change, Relationships, Sensitivity, and a visible Glossary, followed by the collapsed technical appendix. It supersedes the original fixed display order for the correlation matrix and the restriction of strategy contributions to the appendix. It makes no change to estimation, holdings, attribution, stress calculations, public interfaces, or `DisplayConfig`.

- Label the official matrix with the resolved correlation half-life, e.g. **Official Correlation Matrix (126-Day Half-Life)**, explaining that the half-life uses trading days and is not a rolling window. On a copy, start with alphabetical strategy identifiers and cluster distances `sqrt((1-rho)/2)` using SciPy average linkage and optimal leaf ordering. Apply one leaf permutation to both axes; retain every correlation coefficient and the fixed [-1, 1] scale. Bound only distance roundoff, never repair or alter the published matrix. A single strategy needs no clustering. The current-snapshot display order may change through time; the analytical universe order never changes.
- Immediately after Current Diversification, display all precomputed strategy Euler contributions in a horizontal waterfall, descending by signed contribution with identifier tie-breaking. Preserve negative and zero values, accumulate without rounding, and finish with the precomputed portfolio volatility total. Show contributions in annualized volatility points and the total as annualized volatility. Undefined contributions receive a reason instead of a chart. Keep this level decomposition separate from the monthly-change waterfall.
- Place an expanded glossary before the technical appendix. Define portfolio/standalone volatility, standalone risk exposure, DR, effective standalone exposures, zero-correlation volatility, correlation uplift/multiplier, equal-risk correlation reference, model disagreement, Euler/cross-covariance contributions, correlation convergence, volatility multiplier, EWMA half-life, effective sample size, and variance-Shapley attribution. Use the formulas in Appendices B–E and their interpretation limits, including `DR = sqrt(N_eff) / M_R` and the example `0.20 + 0.25 * (1 - 0.20) = 0.40`.
- Apply deliberate Title Case to headings, cards, charts, table headers, and appendix summaries, including notebook markdown headings. Keep prose in sentence case and preserve source identifiers, acronyms, configuration keys, and the existing public `rank_pairs` output values.

Presentation update verification (2026-09-07): `uv sync --all-groups --locked` and `uv run ruff check .` pass; `uv run pytest` passes all 114 tests, including the fresh-kernel notebook and exact HTML/display parity. The notebook test requires local socket access unavailable inside the restrictive sandbox; it passes with that access enabled. SciPy 1.18.1 is locked; data and model modules are unchanged. Independent review passed 60 matrix/portfolio cases across 1–20 strategies, including 32 cases with negative contributions; maximum cumulative-to-total error was 7.11e-15 volatility points, with no analytical mutations or unresolved findings.

Static visual checks covered the complete six-page rendering of the HTML report, the executed notebook's headings and report output, and a fixture with long strategy identifiers, zero allocations and a negative Euler contribution. Matrix values/labels, waterfall annotations, glossary formulas, and section capitalization were legible without overlap. Browser inspection of local file URLs was blocked by browser policy; visual QA used WeasyPrint and Poppler with external resource fetching disabled and no script execution. These temporary QA dependencies and previews are outside the committed dependency set; generated files remain under ignored `reports/`.

Performance on the same AMD Ryzen 9 PRO 8945HS, Linux 7.1.13 and Python 3.13.12: warmed 5,000-date × 10-strategy analysis runs were 1.230, 1.213 and 1.241 seconds (median 1.230 seconds). Fresh-kernel demo execution and HTML export took 2.826 seconds. The report is 324,909 bytes and contains six embedded SVG charts. Both the five-second analysis and thirty-second notebook targets pass. Private `TylerWang1996/qis-dashboards` ownership, privacy and default branch `main` were verified before delivery.

Delivery checkpoint: implementation commit `093d75762d39a842389166060a39da1377715d18` was pushed to private `main`, and `git ls-remote origin refs/heads/main` confirmed the same hash. [GitHub Actions run 34165492534](https://github.com/TylerWang1996/qis-dashboards/actions/runs/34165492534) passed locked installation, Ruff and the complete test suite. This ledger closure is a documentation-only follow-up; its final remote verification is recorded in the task handoff.
